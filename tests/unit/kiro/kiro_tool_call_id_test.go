...
func TestValidateToolCallID(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			result := kiro.ValidateToolCallID(tc.input)
...
func TestSanitizeToolCallID(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:           "whitespace trimmed and generates id",
			input:          "   ",
			shouldGenerate: true,
		},
		{
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			result := kiro.SanitizeToolCallID(tc.input)
			...
			if tc.shouldGenerate {
				...
				assert.True(t, kiro.ValidateToolCallID(result), "case=%s: generated ID should be valid", tc.name)
				assert.True(t, isValidUUIDFormat(result), "case=%s: generated ID should follow UUID format", tc.name)
				return
			}
...
func TestSanitizeToolCallIDUniqueness(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		result := kiro.SanitizeToolCallID(invalidInput)
		require.True(t, kiro.ValidateToolCallID(result), "generated ID should be valid")
...
func TestSanitizeToolCallIDPerformance(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		result := kiro.SanitizeToolCallID(validID)
...
func TestToolCallIDIntegration(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:     "mixed values keeps first valid",
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			result := kiro.SanitizeToolCallID(first)

			if tc.expectGenerated {
				assert.True(t, kiro.ValidateToolCallID(result), "result should be a valid tool_call_id")
