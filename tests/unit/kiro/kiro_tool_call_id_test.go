	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, input := range tc.inputs {
				...
				} else if kiro.ValidateToolCallID(input) {
					sanitized[i] = input // Keep valid inputs unchanged
				} else {
					sanitized[i] = kiro.SanitizeToolCallID(input) // Sanitize invalid inputs
				}
			}
			...
			if tc.expected == "should_be_valid_uuid" {
				...
				assert.True(t, kiro.ValidateToolCallID(result), "result should be valid")
