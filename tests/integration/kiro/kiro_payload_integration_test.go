...
func TestKiroPayloads(t *testing.T) {
	...
	// 4. Run Payload Tests
	testDataDir := filepath.Join(rootDir, "tests/shared/testdata/nonstream")
	apiKey := "test-api-key-1234567890" // Matches config.yaml default
	...
	t.Run("OpenAI Chat Completions", func(t *testing.T) {
		...
		for _, file := range files {
			t.Run(file, func(t *testing.T) {
				payload := loadAndModifyPayload(t, testDataDir, file, "kiro-sonnet")
...
func startServer(t *testing.T, rootDir string) (*exec.Cmd, int, func()) {
	...
	configContent, err := os.ReadFile(origConfigPath)
	require.NoError(t, err)

	// Replace port
	newConfigContent := strings.Replace(string(configContent), "port: 8317", fmt.Sprintf("port: %d", port), 1)
	tempConfigPath := filepath.Join(rootDir, "config_test_temp.yaml")
	err = os.WriteFile(tempConfigPath, []byte(newConfigContent), 0644)
	require.NoError(t, err)
